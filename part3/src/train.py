...
def regression_metrics(y_true, y_pred):
    """Metrics used by the competition, reported on the validation hold-out."""
    return {
...
def main():
    ...
    # Default fixed calibration used for the final Kaggle submission.
    ...
    val_predictions = {
        "global": (raw_rev_val * cr_global, raw_cog_val * cc_global),
        "per_q": (cr_perq_pred, cc_perq_pred),
        "default": (raw_rev_val * C.DEFAULT_CR, raw_cog_val * C.DEFAULT_CC),
    }
    val_metrics = {
        label: {
            "Revenue": regression_metrics(y_val_rev, rev_pred),
            "COGS": regression_metrics(y_val_cog, cog_pred),
        }
        for label, (rev_pred, cog_pred) in val_predictions.items()
    }

    # Keep a validation-best artifact for diagnostics.
    ...
    print(f"  Validation-best artifact: outputs/submissions/submission.csv  Rev={sub.Revenue.mean():,.0f}  "
    ...
    meta = {
        ...
        "final_submission_variant": "default",
