...
[~88 features: calendar + Fourier + Tet windows + per-promo + real promo aggregates]
    │
    ▼
─── Layer 1 — 4 GBM boosters (each: 1 base + 4 quarterly specialists) ───
    LightGBM  ───┐
    XGBoost   ───┤
    CatBoost  ───┤── per-target weighted GBM_blend
    LGBQ-50   ───┘
    │
    ▼
...
5. **Calibration multipliers** — validation-best calibration is kept as a diagnostic artifact, while the final Kaggle upload uses the fixed public-calibrated multipliers `CR=1.26` and `CC=1.32`.
...
├── outputs/
│   ├── submissions/
│   │   ├── submission.csv                   # validation-best calibration artifact
│   │   ├── submission_default.csv           # fixed-calibrated Kaggle output
...
python run.py train      # train pipeline → submission_default.csv → submission_final.csv
...
| `GBM_WEIGHTS_REV` | (0.30, 0.30, 0.15, 0.25) | LGB/XGB/CAT/LGBQ weights for Revenue |
| `GBM_WEIGHTS_COG` | (0.45, 0.20, 0.15, 0.20) | LGB/XGB/CAT/LGBQ weights for COGS |
